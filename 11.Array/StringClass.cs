...
namespace _11.Array
{
    public class StringClass
    {
        ...
        // 따라서, string은 런타임(동작) 당시에 크기가 결정되며 그 크기가 일정하지 않음
        ...
        void Main7()
        {
            ...
            // - 고정소수점 형식 -
            Console.WriteLine($"{0.555:F2}, {0.554:F2}");   // output : 0.56, 0.55  //소수점 fn번째 자리 까지 반올림
        ...
        void Main6()
        {
            ...
            for (int i = 0; i < 10; i++)
            {
                sb.Append((char)65 + i);            // 문자열 붙이기를 정말 해야한다면 이 방식을 사용하자
                sb.Append(":");                     // npc의 대사가 순차적으로 목소리와 같이 나오는 형식의 경우에
                sb.Append(65 + i);                  // 이런 방식으로 사용한다
                sb.AppendLine();
            }
